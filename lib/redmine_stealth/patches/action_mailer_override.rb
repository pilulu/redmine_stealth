  ...
  module Patches
    module ActionMailerOverride
      def deliver_later(options = {})
        return super(options) unless User.current.stealth_enabled || User.current.stealth_mode_active?
      ...
      def deliver_now
        return super unless User.current.stealth_enabled || User.current.stealth_mode_active?
        return unless Rails.logger
